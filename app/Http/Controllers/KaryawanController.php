...
// use App\mnv_iku;
use App\Models\mnv_iku as ModelsMnv_iku;
use App\Models\mnv_ikk as ModelsMnv_ikk;
...
class KaryawanController extends Controller
{
    public function index(Request $request)
    {
        ...
        // // return $data_ikk;
        // return view('pages.karyawan', compact('data_iku'), compact('data_ikk'));

        // $tables = DB::select('SHOW TABLES');
        $tables=DB::select('SELECT mnv_strategi.strategi, mnv_intervensi.intervensi, mnv_instrumen.indikator_intervensi, mnv_instrumen.pemangku_kepentingan1, mnv_iku.nama_indikator AS iku, mnv_ikk.nama_indikator AS ikk, mnv_terpercaya.nama_indikator AS terpercaya, mnv_landscale.nama_indikator AS landscale, mnv_sourceup.nama_indikator AS sourceup, mnv_kdsd.nama_indikator AS kdsd FROM mnv_instrumen, mnv_iku, mnv_intervensi, mnv_strategi, mnv_ikk, mnv_terpercaya, mnv_landscale, mnv_sourceup, mnv_kdsd  WHERE mnv_strategi.id = mnv_intervensi.id_strategi AND mnv_instrumen.id_iku = mnv_iku.id AND mnv_instrumen.id_ikk = mnv_ikk.id AND mnv_instrumen.id_terpercaya = mnv_terpercaya.id AND mnv_instrumen.id_landscale = mnv_landscale.id AND mnv_instrumen.id_sourceup = mnv_sourceup.id AND mnv_instrumen.id_kdsd = mnv_kdsd.id AND mnv_intervensi.id = mnv_instrumen.id_intervensi');
        // return $tables;
